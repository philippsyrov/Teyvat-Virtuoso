...
// Model one selector entry in the bundled or user-saved song library.
struct Song: Codable {
    ...
    let subtitle: String
    // Locate the matching JSON score filename.
    let file: String
    // Mark generated Application Support scores without changing existing public manifests.
    let userProvided: Bool?

    // Keep programmatic song creation concise and backwards-compatible.
    init(id: String, title: String, subtitle: String, file: String, userProvided: Bool? = nil) {
...
final class UserScoreStore {
    ...
    // Load valid locally generated picker entries.
    func loadSongs() -> [Song] {
        ...
        return library.songs.filter { FileManager.default.fileExists(atPath: scoreURL(for: $0.file).path) }
    }
    ...
    func save(title: String, events: [AppScoreEvent]) throws -> Song {
        ...
        // Create the user-visible local picker entry.
        let song = Song(
        ...
        let library = SongLibrary(songs: loadSongs() + [song])
        // Write the manifest only after the score exists.
        try encoder.encode(library).write(to: manifestURL, options: .atomic)
